...
<%@ page import="model.entity.Comunicados"%>
...
<link rel="stylesheet" type="text/css" href="../css/estilo.css" />
	...
	<br>
	<%
		List<Comunicados> comunicados= (List<Comunicados>)request.getAttribute("comunicados");
	%>
	
	
	<div class="container" style="background-color: #FFFFFF;">
		
		<form action="/comunicados/buscar">
		Nuevo Comunicado para los tutores de 
	<select  name="grado">
  <option value="1p" disabled selected>1° primaria</option>
...
<input type="submit" value="Redactar">
		...
		<ul class="list-group" id="myList">
			...
			%>
			<%
			Comunicados p = (Comunicados) comunicados.get(i);
			%>
			<li class="list-group-item"><%=p.getAsunto()%> 
			<a
				href="/comunicados/delete?id=<%=p.getId()%>"
				class="btn btn-outline-danger btn-sm RbtnMargin float-right">Delete
					</button>
			</a> <a href="/comunicados/view?id=<%=p.getId()%>"
	...
	<%
				...
				<h3>No hay Comunicados enviados</h3>
				<%}
	%>
