			...
			if (uSearch.isEmpty()) {
				...
				if (rSearch.isEmpty()) {
					...
					if (aSearch.isEmpty()) {
						...
						String query = "select from " + Homework.class.getName();

						List<Homework> homeworks = (List<Homework>) pm.newQuery(query).execute();

						// pasar la lista al jsp
						req.setAttribute("homeworks", homeworks);

						RequestDispatcher var = getServletContext()
								.getRequestDispatcher("/WEB-INF/Views/Homeworks/index.jsp");
						var.forward(req, resp);
					}
				}
			}
