...
import model.entity.Comunicados;
...
public class ComunicadosControllerIndex extends HttpServlet {
			...
			if (uSearch.isEmpty()) {
				...
				if (rSearch.isEmpty()) {
					...
					if (aSearch.isEmpty()) {
						...
						String query = "select from " + Comunicados.class.getName();

						List<Comunicados> comunicados = (List<Comunicados>) pm.newQuery(query).execute();

						// pasar la lista al jsp
						req.setAttribute("comunicados", comunicados);

						RequestDispatcher var = getServletContext()
								.getRequestDispatcher("/WEB-INF/Views/Comunicados/index.jsp");
						var.forward(req, resp);
					}
				}
			}
